            ...
            <div class='row'>
              <div class="col-md-6">
                ...
                <template v-if='!!resources'>
                  <div class="box box-solid box-default">
                    <div class="box-header">
                      <h3 class="box-title">域名情况(秒/次)&nbsp;
                        /&nbsp;域名数量：<a href='javascript:void(0);' style='color:#2180ec' @click='handleClick($event, 1)'>{{(!resources.domain || !resources.domain.labels) ? 0:resources.domain.labels.length}}</a>
                      </h3>
                      <div class="box-tools pull-right">
                        <button type="button" class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-minus"></i>
                        </button>
              ...
              <div class="col-md-6">
                <template v-if='!!pv'>
                  <div class="box box-solid box-default">
                    <div class="box-header">
                      <h3 class="box-title">页面访问情况(次)／总数：{{pvAmount}}</h3>
                      <div class="box-tools pull-right">
                        <button type="button" class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-minus"></i>
                        </button>
                        <button type="button" class="btn btn-box-tool" data-widget="remove"><i class="fa fa-times"></i></button>
        ...
        <div slot="body">
            <div class='row'>
              <div class="col-md-12">
                ...
                <template v-if='resourcesTableData.length > 8'>
                  <div id='allDomain' class="box box-solid box-default">
                    <div class="box-header">
                    ...
                    <div class="box-body">
                      <data-table
                        :columns="resourcesTable.columns"
  ...
  export default {
    ...
    data() {
      ...
      return {
          ...
          fields: [
            {
              ...
              title: '开始时间',
              type: 'datepicker',
              value: `${start.getFullYear()}-${start.getMonth() + 1}-${start.getDate()}`,
              format: 'yyyy-MM-dd'
            },{
              name: 'query_end',
              title: '结束时间',
        ...
        bigMemoryTable: {
          size: SIZE,
          pageAction: 'getBigMemoryTableData',
          columns: [
            {title: 'used', key: 'used'},
    ...
    watch: {
      pageData(val){
        this.form.fields[2].options = val;
      },
      timeLine(val){
        if(!val){
          return;
        ...
        var range = val.range;
        this.form.fields[1].value = BaseUtil.formatTime(range[0], 'yyyy-M-d');
        this.form.fields[0].value = BaseUtil.formatTime(range[1], 'yyyy-M-d');
        this.timeLineOpt = {
          type: 'line',
          data: val.data
        };
      ...
      resources(val){
        ...
        var _domain = {
          datasets: [
            {
              backgroundColor: domain.datasets[0].backgroundColor,
              label: domain.datasets[0].label,
              type: domain.datasets[0].type,
              data: domain.datasets[0].data.slice(0, 8)
            },{
              backgroundColor: domain.datasets[1].backgroundColor,
              label: domain.datasets[1].label,
              type: domain.datasets[1].type,
              data: domain.datasets[1].data.slice(0, 8)
            },
          ],
          labels: domain.labels.slice(0, 8)
        };
        ...
        setTimeout(() => {
          ...
          labels.forEach((item, index) => {
            this.resourcesTableData.push({
              domain: item,
              dur: domain.datasets[0].data[index],
              num: domain.datasets[1].data[index]
            });
          });
        }, 3000);
      },
      pv(val){
        ...
        this.pvAmount = val.amount;
      }
    },
    methods: {
      handleClick(e, type){
        document.body.scrollTop = $('#allDomain').offset().top;
      },
      addFormData(data){
        var formData = {}, val = null;
        this.form.fields.forEach(function(item){
          val = item.value;
          if(item.name == 'query_start' || item.name == 'query_end'){
            val = Date.parse(val);
          }
          formData[item.name] = val;
        });
        formData['query_end'] += BaseUtil.perDay;
      ...
      doSure(){
        ...
        switch (this.modal.action) {
          case 'resolve':
            this.doJsErrorSolve(this.addFormData({
              id: this.modal.id,
              name: name,
      ...
      trigger(e, name, type) {
        switch (type) {
          case 'project':
            location.href = `index#!/project-item-1?name=${name}`;
            break;
          default:
      ...
      query(){
        var name = BaseUtil.getUrlParam('name');
        if(name){
          var p = this.addFormData({name: name});
          this.getTimeLine(p);
          this.getMemory(p);
          this.getResources(p);
          this.getPv(p);
          this.getJsErrorTableData(this.addFormData({
            name: name,
            cur: 1,
            pageSize: SIZE
          }));
          this.getSlowTimingTableData(this.addFormData({
            name: name,
            cur: 1,
            pageSize: SIZE
          }));
          this.getBigMemoryTableData(this.addFormData({
            name: name,
            cur: 1,
            pageSize: SIZE
          }));
          this.getPages({name:name});
        }
      }
    },
